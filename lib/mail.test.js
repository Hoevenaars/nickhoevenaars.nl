...
test('fillFromMailTemplate sets subject and body, keeps reply subject', () => {
  ...
  assert.deepEqual(fillFromMailTemplate(t, {}), { subject: 'Kennismaking', body: 'Hoi,\n\nBedankt voor je mail.' })
  assert.deepEqual(
    fillFromMailTemplate(t, { isReply: true, currentSubject: 'Re: Offerte', quoted: '\n\n> origineel' }),
    { subject: 'Re: Offerte', body: 'Hoi,\n\nBedankt voor je mail.\n\n> origineel' }
  )
  assert.deepEqual(fillFromMailTemplate(null, { currentSubject: 'Blijft', quoted: 'x' }), { subject: 'Blijft', body: 'x' })
})

test('placeholders autofill known fields and leave the rest', () => {
  const body = 'Hoi [naam],\n\nIk kwam [bedrijfsnaam] tegen.\n\n[opmerking]'
  assert.deepEqual(findMailPlaceholders(body), ['naam', 'bedrijfsnaam', 'opmerking'])
  const values = mailAutofillValues({
    customer: { company_name: 'Acme', website: 'https://acme.nl' },
    contact: { name: 'Ada Lovelace', email: 'ada@acme.nl' },
    now: new Date('2026-08-27T12:00:00Z')
  })
  assert.equal(values.naam, 'Ada')
  assert.equal(values.bedrijfsnaam, 'Acme')
  const filled = applyMailPlaceholders(body, values)
  assert.equal(filled, 'Hoi Ada,\n\nIk kwam Acme tegen.\n\n[opmerking]')
  assert.deepEqual(leftoverMailPlaceholders(filled), ['opmerking'])
  assert.deepEqual(leftoverMailPlaceholders('Klaar\n\n> Hoi [naam]'), [])
  assert.equal(leftoverFieldsError(['naam', 'bedrijfsnaam']), 'Vul nog in: [naam], [bedrijfsnaam]')
})

test('appendMailFooter once and fillFromMailTemplate uses it', () => {
  assert.equal(appendMailFooter('Hoi', 'Groet,\nNick'), 'Hoi\n\nGroet,\nNick')
  assert.equal(appendMailFooter('Hoi\n\nGroet,\nNick', 'Groet,\nNick'), 'Hoi\n\nGroet,\nNick')
  const t = { subject: 'Hoi [naam]', body: 'Hoi [naam],' }
  assert.deepEqual(
    fillFromMailTemplate(t, { values: { naam: 'Ada' }, footer: 'Nick' }),
    { subject: 'Hoi Ada', body: 'Hoi Ada,\n\nNick' }
  )
})
