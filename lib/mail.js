import { namesOf, normalizeGender } from './contacts.js'
...
export const DEFAULT_MAIL_FOOTER = 'Met vriendelijke groet,\nNick Hoevenaars'

const LEADING_GREETING_RE = /^(?:hi|hoi|hallo|beste)(?:\s+[^,\n]+)?,?[ \t]*(?:\n+|$)/i

export function mailGreeting(person, style = 'informeel') {
  if (style === 'formeel') {
    const gender = normalizeGender(person?.gender)
    const last = namesOf(person).last_name
    if (gender && last) return `Beste ${gender} ${last},`
    if (gender) return `Beste ${gender},`
    if (last) return `Beste ${last},`
    return 'Beste,'
  }
  const first = namesOf(person).first_name
  return first ? `Hi ${first},` : 'Hi,'
}

export function stripLeadingGreeting(body) {
  return String(body || '').replace(LEADING_GREETING_RE, '')
}

export function applyMailGreeting(body, greeting) {
  const g = String(greeting || '').trim()
  const rest = stripLeadingGreeting(body).replace(/^\n+/, '')
  if (!g) return rest
  if (!rest) return `${g}\n\n`
  return `${g}\n\n${rest}`
}

export function appendMailFooter(body, footer = DEFAULT_MAIL_FOOTER) {
...
export function fillFromMailTemplate(template, { isReply = false, currentSubject = '', quoted = '', footer = DEFAULT_MAIL_FOOTER, greeting = '' } = {}) {
  let body = template?.body || ''
  if (greeting) body = applyMailGreeting(body, greeting)
  if (!template) {
    return { subject: currentSubject, body: appendMailFooter(applyMailGreeting('', greeting), footer) + quoted }
  }
  return {
    subject: (isReply && currentSubject) ? currentSubject : (template.subject || ''),
    body: appendMailFooter(body, footer) + quoted
  }
}
...
export function recentDuplicateOutbound(emails, { to, subject, text, attachments }, withinMs = 20000, now = Date.now()) {
  const dest = parseAddress(to).email
  const sub = String(subject || '').trim()
  const body = String(text || '')
  const files = (attachments || []).map((a) => String(a?.filename || '')).join('|')
  if (!dest) return null
  ...
  return (emails || []).find((m) => {
    ...
    const existingFiles = (m.attachments || []).map((a) => String(a?.filename || '')).join('|')
